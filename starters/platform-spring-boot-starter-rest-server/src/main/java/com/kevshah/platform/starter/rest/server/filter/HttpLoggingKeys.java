 ...
 *
 * <p>Every SLF4J {@code addKeyValue} call in {@link StandardRequestResponseLoggingFilter} must reference a constant
 * from this class so that key names are defined in exactly one place and remain consistent across request and response
 * entries.
 *
 * <p>All keys follow the pattern {@code platform.rest-server.http.<concern>}.
 */
public final class HttpLoggingKeys {
    ...
    /** Request URI path (e.g. {@code "/api/orders/42"}). */
