...
public record LoggingProperties(
    ...
    ///
    /// @param enabled Set to `true` to emit a log entry for the request. When `false` or `null`, no request log entry is emitted.
    ...
    ///
    /// @param enabled Set to `true` to emit a log entry for the response. When `false` or `null`, no response log entry is emitted.
    ...
    ///
    /// @param enabled Set to `true` to capture and include headers in the log entry. When `false` or `null`, no headers are logged.
    /// @param include List of header names to include in the log entry. When empty or `null`, all headers are included (subject to `enabled`).
    /// @param exclude List of header names to exclude from the log entry. When empty or `null`, no headers are excluded. If a header is listed in both `include` and `exclude`, it is excluded.
    public record HeadersConfig(
        ...
        /// Header name matching is case-insensitive. When `include` is empty or `null`, all headers are included (subject to `enabled`), except those listed in `exclude`.
