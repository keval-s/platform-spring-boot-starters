...
/// `default-query-params` are merged on top of these values.
...
/// @param endpoints          Named endpoint definitions for this client. Each key becomes the logical name used to look
/// up the endpoint via `PlatformRestClientRegistry.getEndpoint(clientName, endpointName)`.
...
/// `logging` settings are merged on top of these values.
